...
class PyLouvain:
    ...
    def from_file(cls, path):
        ...
        nodes_, edges_,t= in_order(nodes, edges)
        print("%d nodes, %d edges" % (len(nodes_), len(edges_)))
        return cls(nodes_, edges_,t)
    
    @classmethod
    def from_db(cls,dbm,sql):
        lines = dbm.executeSelect(sql)
        t = {}
        nodes = {}
        edges = []
        for n in lines:
    ...
    '''
    def __init__(self, nodes, edges,t):
        self.node_map = {v:k for k,v in t.items()}
        self.nodes = nodes
        self.edges = edges
    ...
    def apply_method(self):
        ...
        while 1:
            ...
            if q == best_q:
                break
            network = self.second_phase(network, partition)#重新构造网络
    ...
    def first_phase(self, network):
        ...
        best_partition = self.make_initial_partition(network)#每个节点都有个社区
        while 1:
            improvement = 0
            for node in network[0]:
                ...
                best_shared_links = 0
                for e in self.edges_of_node[node]:#计算社团内部的边
                    if e[0][0] == e[0][1]:#如果节点自己指向自己则继续，不计算此边在内
                        continue
                    if e[0][0] == node and self.communities[e[0][1]] == node_community or e[0][1] == node and self.communities[e[0][0]] == node_community:
                        best_shared_links += e[1]#如果节点和邻居节点的边都在社团内，则社团内部边加1
                ...
                for neighbor in self.get_neighbors(node):
                    community = self.communities[neighbor]#邻居的社区编号
